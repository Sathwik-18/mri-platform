...
def build_clinician_report(
    ...
    try:
        ...
        pdf.cell(box_width, 6, sanitize_for_pdf(f"{pred_name} ({prediction})"), 0, 0, 'C')
        ...
        if probabilities and classes:
            ...
            for cls, prob in zip(classes, probabilities):
                prob_parts.append(f"{cls}: {prob*100:.1f}%")

            pdf.key_value_pair("Probabilities", " | ".join(prob_parts), 50)
            pdf.key_value_pair("Primary Confidence", f"{confidence*100:.1f}%", 50)
            pdf.ln(4)
...
def _add_volume_table(pdf: ClinicianPDFReport, ml_results: Dict):
    """Add volumetric measurements table."""
    volumes = [
    ...
    pdf.cell(45, 6, "Normal Range", 1, 0, 'C', True)
    ...
    for name, value, norm_key in volumes:
        ...
        if value < min_v:
            ...
            status = 'Normal'
            pdf.set_text_color(*pdf.color_normal)

        pdf.set_text_color(*pdf.text_color_normal)
        pdf.cell(55, 6, sanitize_for_pdf(name), 1, 0, 'L')
        pdf.cell(35, 6, f"{value:.2f} {unit}", 1, 0, 'C')
        pdf.cell(45, 6, f"{min_v}-{max_v} {unit}", 1, 0, 'C')
        ...
        pdf.cell(35, 6, sanitize_for_pdf(status), 1, 1, 'C')
        pdf.set_text_color(*pdf.text_color_normal)
