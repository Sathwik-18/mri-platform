...
def build_technical_report(
    ...
    try:
        ...
        if probabilities and classes:
            pdf.ln(2)
            prob_str = " | ".join([f"{c}: {p*100:.2f}%" for c, p in zip(classes, probabilities)])
            pdf.key_value_pair("Confidence Distribution", prob_str, 50)
            pdf.key_value_pair("Primary Confidence", f"{confidence*100:.2f}%", 50)
        ...
        if affected_regions:
            ...
            pdf.cell(50, 6, "Region", 1, 0, 'C', True)
            pdf.cell(40, 6, "Severity", 1, 0, 'C', True)
            pdf.cell(0, 6, "Observation", 1, 1, 'C', True)

            pdf.set_font('Helvetica', '', 9)
            for region in affected_regions:
                pdf.cell(50, 6, sanitize_for_pdf(region['name']), 1, 0, 'L')
                pdf.cell(40, 6, sanitize_for_pdf(region.get('severity', 'N/A')), 1, 0, 'C')
                pdf.cell(0, 6, sanitize_for_pdf(region.get('description', '')), 1, 1, 'L')
...
def _add_detailed_volume_table(pdf: TechnicalPDFReport, ml_results: Dict):
    """Add detailed volumetric measurements table."""
    volumes = [
    ...
    pdf.set_fill_color(230, 230, 230)
    pdf.cell(50, 6, "Structure", 1, 0, 'C', True)
    pdf.cell(30, 6, "Measured", 1, 0, 'C', True)
    pdf.cell(35, 6, "Normal Range", 1, 0, 'C', True)
    pdf.cell(25, 6, "Status", 1, 0, 'C', True)
    pdf.cell(0, 6, "Deviation", 1, 1, 'C', True)
    ...
    for name, value, norm_key in volumes:
        ...
        pdf.set_text_color(*pdf.text_color_normal)
        pdf.cell(50, 5, sanitize_for_pdf(name), 1, 0, 'L')
        pdf.cell(30, 5, f"{value:.2f}", 1, 0, 'C')
        pdf.cell(35, 5, f"{min_v}-{max_v}", 1, 0, 'C')

        pdf.set_text_color(*status_color)
        pdf.cell(25, 5, sanitize_for_pdf(status), 1, 0, 'C')
        pdf.set_text_color(*pdf.text_color_normal)
        pdf.cell(0, 5, sanitize_for_pdf(deviation), 1, 1, 'C')
