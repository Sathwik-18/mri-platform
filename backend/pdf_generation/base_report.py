...
class BaseMRIReport(FPDF):
    ...
    def __init__(self, *args, **kwargs):
        ...
        # Color scheme
        self.primary_color = (52, 73, 94)       # Dark blue-gray
        self.secondary_color = (74, 144, 226)   # Blue
        ...
        # Status colors
        self.color_normal = (46, 204, 113)      # Green
        self.color_warning = (243, 156, 18)     # Orange
        self.color_danger = (231, 76, 60)       # Red
        self.color_info = (52, 152, 219)        # Blue

        # Disease colors
        self.disease_colors = {
            'CN': (46, 204, 113),    # Green - Cognitively Normal
            'MCI': (243, 156, 18),   # Amber/Yellow - Mild Cognitive Impairment
            'AD': (231, 76, 60)      # Red - Alzheimer's Disease
        }
    ...
    def header(self):
        ...
        try:
            ...
            title = sanitize_for_pdf(self.report_title)
            title_w = self.get_string_width(title) + 6
            self.set_x((self.w - title_w) / 2)
            self.set_text_color(*self.secondary_color)
            self.cell(title_w, 10, title, border=0, align='C')
            self.ln(12)
    ...
    def add_hospital_header(self, hospital_data: Optional[Dict] = None):
        ...
        try:
            ...
            if hospital_data.get('address'):
                address_parts.append(hospital_data['address'])
            if hospital_data.get('city'):
                address_parts.append(hospital_data['city'])
            if hospital_data.get('state'):
                address_parts.append(hospital_data['state'])
            if hospital_data.get('pincode'):
                address_parts.append(hospital_data['pincode'])

            if address_parts:
                self.cell(0, 5, sanitize_for_pdf(', '.join(address_parts)), 0, 1, 'C')

            # Contact
            contact_parts = []
            if hospital_data.get('phone'):
                contact_parts.append(f"Phone: {hospital_data['phone']}")
            if hospital_data.get('email'):
                contact_parts.append(f"Email: {hospital_data['email']}")

            if contact_parts:
                self.cell(0, 5, sanitize_for_pdf(' | '.join(contact_parts)), 0, 1, 'C')
    ...
    def section_title(self, title: str):
        ...
        try:
            ...
            self.cell(0, 8, " " + sanitize_for_pdf(title), border='B', align='L', fill=True)
            self.ln(6)
            self.set_text_color(*self.text_color_normal)
        except Exception as e:
    ...
    def add_explanation_box(self, title: str, items: List, bg_color: Tuple = None):
        """Add an explanation box with bullet points."""
        try:
            ...
            for item in items:
                ...
                if is_bullet:
                    ...
                    self.multi_cell(box_width - 17, 5, sanitize_for_pdf(text), 0, 'L')
                else:
                    self.set_x(self.l_margin + 5)
                    self.multi_cell(box_width - 10, 5, sanitize_for_pdf(text), 0, 'L')
        ...
        try:
            ...
            for point in text_list:
                ...
                self.multi_cell(self.w - self.l_margin - self.r_margin - 10, 4.5,
                               f"* {sanitize_for_pdf(point)}", 0, 'L')
                self.ln(1)

            end_y = self.get_y()
            box_height = end_y - start_y + 4

            self.rect(self.l_margin, start_y, self.w - self.l_margin - self.r_margin, box_height, 'D')
            self.set_y(end_y + 4)
            self.set_text_color(*self.text_color_normal)
